...
class StratifiedMLR:
    ...
    def predict_future_prices(self, target_month, target_year, cpi_estimate=None, fuel_price_estimate=None):
        ...
        for _, product_row in latest_month_data.iterrows():
            ...
            # Scale features using the scaler from the best model
...
if __name__ == "__main__":
    ...
    mlr_analyzer.is_fitted = True
    
    print("\n" + "="*60)
    print("PREDICTING FOOD PRICES FOR DECEMBER 2025")
    print("="*60)
    
    # Predict prices for December 2025
    predictions_2025 = mlr_analyzer.predict_future_prices('December', 2025)
    
    if predictions_2025 is not None:
        # Analyze prediction confidence
        confident_predictions = mlr_analyzer.analyze_prediction_confidence(predictions_2025)
        
        # Save predictions to CSV
        predictions_2025.to_csv('december_2025_predictions.csv', index=False)
        print(f"\nPredictions saved to 'december_2025_predictions.csv'")
        
        # Show top predictions by category
        print(f"\n=== TOP PREDICTIONS BY PRODUCT CATEGORY ===")
        
        # Group by product type (simple categorization based on product names)
        predictions_2025['Category'] = predictions_2025['Product'].apply(
            lambda x: 'Vegetables' if any(veg in x.lower() for veg in ['carrot', 'potato', 'onion', 'tomato', 'lettuce']) else
                     'Fruits' if any(fruit in x.lower() for fruit in ['apple', 'banana', 'orange', 'grape', 'berry']) else
                     'Dairy' if any(dairy in x.lower() for dairy in ['milk', 'cheese', 'yogurt', 'butter']) else
                     'Meat' if any(meat in x.lower() for meat in ['beef', 'chicken', 'pork', 'fish']) else
                     'Grains' if any(grain in x.lower() for grain in ['bread', 'rice', 'wheat', 'flour']) else
                     'Other'
        )
        
        category_summary = predictions_2025.groupby('Category').agg({
            'Predicted_Price': ['mean', 'min', 'max', 'count'],
            'Prediction_Std': 'mean'
        }).round(2)
        
        print(category_summary.to_string())
    
    print("\nAnalysis complete! Check 'mlr_stratified_results.png' for visualizations and 'december_2025_predictions.csv' for predictions.")
